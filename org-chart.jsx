...
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, LineChart, AreaChart, Area, CartesianGrid, Legend, ReferenceLine } from "recharts";
import { Search, ChevronDown, ChevronRight, Users, Building2, MapPin, TrendingUp, AlertTriangle, MessageSquare, GripVertical, Play, Pause, SkipBack, SkipForward, Download, Plus, Eye, EyeOff, Filter, BarChart3, Clock, Zap, Target, Layers, Globe, ChevronLeft, X, Check, Flag, Edit3, Trash2, Move, ArrowRight, Info, AlertCircle, Activity, GitMerge, AlertOctagon } from "lucide-react";
...
function generateSemiCompany() {
  ...
  mgrEmps.forEach(mgr => {
    ...
    for (let i = 0; i < span; i++) {
      ...
      employees.push({
        ...
        location: Math.random() < 0.78 ? mgr.location : rn(LOCATIONS),
  ...
  // Weird #1: One Director reports to a peer Director instead of their VP.
  // Their VP left 8 months ago; nobody has formally reassigned them yet.
  if (dirEmps.length >= 6) {
    const orphan = dirEmps[dirEmps.length - 2];
    const anchor = dirEmps[1];
    orphan.managerId = anchor.id;
...
function buildTree(employees, activeOnly = true) {
  ...
  filtered.forEach(e => {
    const node = map[e.id];
    if (!e.managerId || !map[e.managerId]) { root = node; }
    else if (map[e.managerId]) { map[e.managerId].children.push(node); }
  });
  function countReports(n) {
    n._directReports = n.children.length;
    n._totalReports = n.children.reduce((s, c) => s + 1 + countReports(c), 0);
...
function detectHotspots(tree) {
  ...
  function walk(node, depth = 0) {
    ...
    const recentHires = node.children.filter(c => {
      const months = (new Date() - new Date(c.startDate)) / (1000*60*60*24*30);
      return months < 6;
    });
...
// ─── INSIGHT ENGINE ───
const TENURE_MS = (d) => (new Date() - new Date(d)) / (1000 * 60 * 60 * 24 * 30); // months
...
function OrgOverview({ insights, score, breakdown, employees, tree }) {
  ...
  return (
    <div className="h-full overflow-auto p-6" style={{ background: "#f8fafc" }}>
      <p className="text-sm text-gray-500 mb-6">Click any insight card on the left for a narrative deep-dive, or use the filters above to change the analysis lens.</p>

      {/* Health score breakdown */}
      <div className="bg-white rounded-xl p-5 mb-5 border border-gray-100" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        <h3 className="text-sm font-bold text-gray-700 mb-4">Health Score Breakdown</h3>
      ...
      <div className="bg-white rounded-xl p-5 border border-gray-100" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        <h3 className="text-sm font-bold text-gray-700 mb-4">Department Health Map</h3>
        <div className="grid grid-cols-2 gap-2">
          {depts.map(({ dept: d, hc, crits, warns, health }) => {
            ...
            return (
              <div key={d} className="flex items-center gap-2 p-2.5 rounded-xl transition-colors" style={{ background: `${color}09`, border: `1.5px solid ${color}22` }}>
...
function AnalyticsView() {
  const { employees, tree, setView, setFocusRoot, setInsightHighlightIds } = useContext(AppCtx);
  ...
  const visibleInsights = useMemo(() =>
    catFilter === "all" ? allInsights : allInsights.filter(i => i.category === catFilter)
  , [allInsights, catFilter]);
  ...
  const zoomToInsight = useCallback((insight) => {
    ...
    setView("org-chart");
  }, [tree, setFocusRoot, setInsightHighlightIds, setView]);
    ...
    <div className="h-full flex flex-col overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-3 flex items-center gap-6 shrink-0 flex-wrap" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <div>
            <div className="text-xs text-gray-400 font-medium">Org Health Score</div>
        ...
        <div className="flex gap-3 border-l border-gray-100 pl-6 shrink-0">
          {[["critical","🔴","#dc2626","#fef2f2"], ["warning","⚠️","#d97706","#fffbeb"], ["info","💡","#2563eb","#eff6ff"]].map(([sev, icon, color, bg]) => {
            ...
            return (
              <div key={sev} className="text-center px-3 py-1 rounded-lg cursor-pointer transition-all" style={{ background: bg }}
                   onClick={() => setCatFilter("all")}>
                <div className="text-xl font-black" style={{ color }}>{n}</div>
                <div className="text-gray-500 capitalize" style={{ fontSize: 10 }}>{sev}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex items-center gap-2 border-l border-gray-100 pl-6 flex-wrap ml-auto">
          ...
          {["location","bg","fn","dept"].map(field => {
            const labels = { location: "Location", bg: "Business Group", fn: "Function", dept: "Department" };
            return (
...
function FlightRiskView() {
  const { employees } = useContext(AppCtx);
  ...
  return (
    <div className="p-6 h-full overflow-y-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Flight Risk Dashboard</h2>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: "High Risk", count: highRisk, pct: Math.round(highRisk/ranked.length*100), color: "#dc2626", bg: "#fef2f2" },
          { label: "Medium Risk", count: medRisk, pct: Math.round(medRisk/ranked.length*100), color: "#d97706", bg: "#fffbeb" },
          { label: "Low Risk", count: lowRisk, pct: Math.round(lowRisk/ranked.length*100), color: "#16a34a", bg: "#f0fdf4" },
        ].map(k => (
          <div key={k.label} className="rounded-xl p-4 border" style={{ background: k.bg, borderColor: k.color+"33" }}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        <div className="overflow-auto max-h-80">
          <table className="w-full text-xs">
            ...
            <tbody>
              {ranked.filter(e => e.risk !== "low").map(e => {
                return (
                  <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-gray-800">{e.first} {e.last}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2 text-gray-500 text-xs">{e.dept}</td>
                    <td className="px-4 py-2 text-gray-500 text-xs">{e.level}</td>
...
const OrgNode = memo(function OrgNode({ node, depth = 0 }) {
  ...
  const nodeOpacity = hasHighlightSet
  ...
  return (
    <div className="flex flex-col items-center" style={{ opacity: nodeOpacity, transition: "opacity 0.25s ease" }}>
      <div
        id={`node-${node.id}`}
        draggable
        onDragStart={(e) => { e.stopPropagation(); teamDragRef.current = dragMode === "team"; setDragNode(node.id); e.dataTransfer.effectAllowed = "move"; }}
        onDragEnd={() => { setDragNode(null); setDragOver(null); teamDragRef.current = false; }}
        ...
        style={{
          background: isBeingDragged ? "#eff6ff" : isDragTarget ? "#f0fdf4" : isDetailOpen ? "#fffbeb" : "white",
          borderRadius: 10,
          padding: "8px 12px",
          minWidth: 180,
          maxWidth: 220,
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
            style={{ background: color, outline: showFlightRisk && riskLabel ? `2.5px solid ${RISK_COLORS[riskLabel]}` : "none", outlineOffset: 1 }}>
            {node.first[0]}{node.last[0]}
          </div>
          <div className="min-w-0">
            <div className="text-xs font-semibold text-gray-900 truncate">{node.first} {node.last}</div>
        ...
        <div className="flex items-center gap-1 mt-1.5" style={{ fontSize: 9 }}>
          ...
          <span className="text-gray-400 flex items-center gap-0.5"><MapPin size={8}/>{node.location.split(" ")[0]}</span>
          {hasChildren && <span className="text-gray-400 ml-auto flex items-center gap-0.5"><Users size={8}/>{node._totalReports}</span>}
        </div>
      ...
      {hasChildren && isExpanded && (
        <div className="relative mt-6">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-4 bg-gray-200" style={{ top: -16 }}/>
          {node.children.every(c => c.children.length === 0) && node.children.length >= 3 ? (
            /* Compact 2-column grid for leaf-only children — keeps tree narrow */
            <div className="grid gap-1.5 pt-1" style={{ gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
              {node.children.map(child => (
                <OrgNode key={child.id} node={child} depth={depth + 1}/>
              ))}
            </div>
          ) : (
            /* Normal horizontal fan layout */
            <>
              {node.children.length > 1 && (
                <div className="absolute top-0 bg-gray-200" style={{ height: 1, left: `calc(${100/(node.children.length*2)}%)`, right: `calc(${100/(node.children.length*2)}%)`, top: -1 }}/>
              )}
...
function TimelineView() {
  ...
  // All months — used for reference lines and jump buttons
  const allMonthlyData = useMemo(() => months.map((m, i) => {
    const active = getActiveAtDate(employees, m);
    const point = { month: m.toLocaleDateString("en-US", { year: "2-digit", month: "short" }), count: active.length, idx: i };
    if (dimField) dimValues.forEach(v => { point[v] = active.filter(e => e[dimField] === v).length; });
    return point;
  }), [employees, months, dimField, dimValues]);

  // Visible slice — grows as the scrubber moves
  const monthlyData = useMemo(() => allMonthlyData.slice(0, visibleIdx + 1), [allMonthlyData, visibleIdx]);

  const hiresPerMonth = useMemo(() => months.slice(0, visibleIdx + 1).map((m) => {
    ...
    return { month: m.toLocaleDateString("en-US", { year: "2-digit", month: "short" }), hires, departures: -deps };
  }), [employees, months, visibleIdx]);
  ...
  const milestones = useMemo(() => [500, 1000, 1500].map(n => {
    const mIdx = allMonthlyData.findIndex(m => m.count >= n);
    return mIdx >= 0 ? { n, mIdx, month: allMonthlyData[mIdx].month } : null;
  }).filter(Boolean), [allMonthlyData]);
  ...
  const interval = Math.max(1, Math.floor(monthlyData.length / 8));
  ...
  return (
    ...
    <div className="p-6 h-full overflow-y-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Company Growth Timeline</h2>
      ...
      {/* Growth chart — grows as timeline scrubs */}
      <div className="bg-white rounded-xl p-4 mb-6 border border-gray-100" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <h3 className="text-sm font-bold text-gray-700">Headcount Over Time</h3>
          <div className="flex gap-1 flex-wrap">
            {[["total","Total"], ["location","Location"], ["bg","Business Group"], ["fn","Function"], ["dept","Department"]].map(([k, label]) => (
        ...
        <ResponsiveContainer width="100%" height={220}>
          {growthDimension === "total" ? (
            <AreaChart data={monthlyData} margin={{ top: 16, right: 8, bottom: 0, left: 0 }}>
              <defs>
              ...
              <Area type="monotone" dataKey="count" stroke="#2563eb" fill="url(#totalGrad)" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} isAnimationActive={false}/>
            </AreaChart>
          ) : (
            <AreaChart data={monthlyData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
              ...
              {dimValues.map((v, i) => (
                <Area key={v} type="monotone" dataKey={v} stackId="a"
                  ...
                  fillOpacity={0.75} strokeWidth={1.5} dot={false} isAnimationActive={false}/>
              ))}
            </AreaChart>
          )}
        </ResponsiveContainer>
      ...
      {/* Hires / departures — also grows */}
      <div className="bg-white rounded-xl p-4 mb-6 border border-gray-100" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <h3 className="text-sm font-bold text-gray-700 mb-3">Monthly Hires & Departures</h3>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={hiresPerMonth} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/>
            <XAxis dataKey="month" tick={{ fontSize: 9 }} interval={interval}/>
            <YAxis tick={{ fontSize: 10 }}/>
            <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid #e2e8f0" }}/>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          <h3 className="text-sm font-bold text-gray-700 mb-3">
            {growthDimension === "location" ? "Location" : growthDimension === "bg" ? "Business Group" : growthDimension === "fn" ? "Function" : "Department"} Breakdown
          </h3>
...
function DashboardView() {
  const { employees, activeEmployees, tree, hotspots, dashTab, setDashTab, navigateTo, setView } = useContext(AppCtx);
  ...
  const levelData = useMemo(() => Object.entries(activeEmployees.reduce((acc, e) => { acc[e.level]    = (acc[e.level]    || 0) + 1; return acc; }, {})).map(([name,value]) => ({ name, value })), [activeEmployees]);
  ...
  const { engRatio, dvRatio, gaRatio, avgTenure, last90Hires, last90Departures } = useMemo(() => {
    ...
    const eng  = activeEmployees.filter(e => TECH_DEPTS.has(e.dept)).length;
    const des  = activeEmployees.filter(e => e.dept === "IC Design").length;
    const dv   = activeEmployees.filter(e => e.dept === "Verification").length;
    const ga   = activeEmployees.filter(e => e.fn === "G&A").length;
  ...
  const coverageData = useMemo(() => {
    return BUS_GROUPS.map(bg => {
      ...
      COV_ROLES.forEach(role => {
        row[role] = activeEmployees.filter(e => e.bg === bg && e.dept === role).length;
      });
      return row;
    });
  }, [activeEmployees]);
  ...
  return (
    <div className="p-6 h-full overflow-y-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <h2 className="text-xl font-bold text-gray-900 mb-1">Dashboards & Insights</h2>
      ...
      {dashTab === "executive" && (
        <div>
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              { label: "Active Headcount", value: activeEmployees.length, icon: Users, color: "#2563eb" },
              { label: "Avg Tenure (yrs)", value: avgTenure, icon: Clock, color: "#059669" },
              { label: "Hires (90 days)", value: `+${last90Hires}`, icon: TrendingUp, color: "#16a34a" },
              { label: "Departures (90d)", value: last90Departures, icon: AlertCircle, color: "#ef4444" },
            ].map((kpi, i) => (
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-white rounded-xl p-4 border border-gray-100">
              <h3 className="text-sm font-bold text-gray-700 mb-3">Headcount by Department</h3>
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={deptData.slice(0, 10)} layout="vertical" margin={{ left: 80 }}>
                  <XAxis type="number" tick={{ fontSize: 10 }}/>
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={75}/>
                  <Tooltip contentStyle={{ fontSize: 11 }}/>
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {deptData.slice(0, 10).map((d, i) => <Cell key={i} fill={d.fill}/>)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="bg-white rounded-xl p-4 border border-gray-100">
              <h3 className="text-sm font-bold text-gray-700 mb-3">Business Group Mix</h3>
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-xl p-4 border border-gray-100">
              <h3 className="text-sm font-bold text-gray-700 mb-3">By Location</h3>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={locData} layout="vertical" margin={{ left: 70 }}>
                  <XAxis type="number" tick={{ fontSize: 10 }}/>
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={65}/>
                  <Tooltip contentStyle={{ fontSize: 11 }}/>
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {locData.map((_, i) => <Cell key={i} fill={locColors[i % locColors.length]}/>)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="bg-white rounded-xl p-4 border border-gray-100">
              <h3 className="text-sm font-bold text-gray-700 mb-3">By Function</h3>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
      ...
      {dashTab === "semi" && (
        <div>
          ...
          <div className="bg-white rounded-xl p-4 border border-gray-100 mb-4">
            <h3 className="text-sm font-bold text-gray-700 mb-3">Critical Semiconductor Function Coverage</h3>
            <div className="space-y-2">
      ...
      {dashTab === "managers" && (
        <div>
          <div className="bg-white rounded-xl p-4 border border-gray-100 mb-4">
            <h3 className="text-sm font-bold text-gray-700 mb-3">Span of Control Distribution</h3>
            <ResponsiveContainer width="100%" height={160}>
      ...
      {dashTab === "coverage" && (
        <div>
          <p className="text-sm text-gray-500 mb-4">Headcount by Business Group × semiconductor discipline. Red = below minimum viable coverage.</p>
                  ...
                  return (
                    <tr key={row.bg} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      {COV_ROLES.map(role => {
                        ...
                        return (
                          <td key={role} className="text-center px-2 py-2 font-bold transition-colors" style={{ background: bg, color: txt }}>
                            {count === 0 ? "—" : count}
...
function ScenarioView() {
  const { employees, setEmployees, scenarios, setScenarios, scenarioName, setScenarioName,
          undoStack, undo, scenarioNewName, setScenarioNewName } = useContext(AppCtx);
  ...
  const deptFnMap = useMemo(() => {
    const m = {};
    activeEmps.forEach(e => { if (!m[e.dept]) m[e.dept] = e.fn; });
    return m;
  }, [activeEmps]);
  ...
  return (
    <div className="p-6 h-full overflow-y-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="flex gap-1 mb-5 border-b border-gray-100 pb-2">
        {STABS.map(t => (
          <button key={t.id} onClick={() => setScenarioTab(t.id)}
      ...
      {scenarioTab === "scenarios" && (
        <>
          ...
          <div className="flex gap-2 mb-5">
            <input value={scenarioNewName} onChange={e => setScenarioNewName(e.target.value)} placeholder="New scenario name..." className="text-sm border border-gray-200 rounded-lg px-3 py-2 flex-1 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            <button onClick={() => {
                  ...
                  return (
                    <div key={r.id} className="bg-white border border-gray-100 rounded-xl p-4" style={{ borderLeftWidth: 3, borderLeftColor: color }}>
                      ...
                      {[
                        ...
                        { label: "Functions", field: "fns",         opts: RECRUITER_FNS,          colorMap: FN_COLORS   },
...
function DetailPanel() {
  const {
    ...
    setFocusRoot, setDragMode, getNodeColor,
    ...
    <div className="fixed right-0 top-0 bottom-0 w-96 bg-white shadow-2xl z-50 overflow-y-auto border-l border-gray-100" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
          {[
            ["Department", node.dept], ["Business Group", node.bg], ["Function", node.fn],
            ["Location", node.location], ["Level", node.level], ["Status", node.status],
        ...
        <div className="border-t pt-3">
          ...
          <div className="flex flex-wrap gap-1">
            <button onClick={() => { setFocusRoot(node.id); setDetailPanel(null); }} className="text-xs bg-gray-100 text-gray-700 px-3 py-1.5 rounded hover:bg-gray-200 transition-colors flex items-center gap-1"><Eye size={10}/>Focus Subtree</button>
...
function TreemapOrgView() {
  const { activeEmployees, setOrgViewMode, setFocusRoot, tree } = useContext(AppCtx);
          ...
          return (
            <div
              ...
              onClick={() => {
                ...
                if (nodes[0]) setFocusRoot(nodes[0].id);
                setOrgViewMode("tree");
              }}
...
function SunburstOrgView() {
  const { tree, setFocusRoot, displayRoot } = useContext(AppCtx);
  const [hovered, setHovered] = useState(null);
          ...
          return (
            <g key={i}>
              <path d={describeArc(a.startAngle, a.endAngle, innerR, outerR)}
                ...
                onMouseLeave={() => setHovered(null)}
                onClick={() => { setFocusRoot(a.node.id); }}
              />
              {showLabel && (
...
function computeProductLineAssignments(employees) {
  const active = employees.filter(e => e.status === "Active");
  const pl1 = [], pl2 = [];
  const CORP_ONLY = new Set(["Finance","HR","Legal","IT","Facilities"]);
  active.forEach(e => {
    if (["Automotive BU","Industrial BU"].includes(e.bg))              pl1.push(e);
    else if (["Data Center BU","Consumer BU","IoT BU"].includes(e.bg)) pl2.push(e);
    else if (e.bg === "Corporate" && !CORP_ONLY.has(e.dept))
...
function ProductPipelineView() {
  const { employees, flightRisks } = useContext(AppCtx);
  ...
  const stageEmps = useCallback((stageId, plId) => {
    ...
    return stage ? (assignments[plId] || []).filter(e => stage.depts.includes(e.dept)) : [];
  }, [assignments]);

  // Full grid: stageId → plId → { emps, warnings, status }
  const grid = useMemo(() => {
    const g = {};
    PIPELINE_STAGES.forEach(stage => {
      ...
      PRODUCT_LINES.forEach(pl => {
        ...
        const status = emps.length === 0 ? "critical" : emps.length === 1 ? "spof" : warnings.some(w => w.type === "understaffed") ? "warning" : hiRisk.length ? "flight" : "healthy";
        g[stage.id][pl.id] = { emps, warnings, status };
      });
    });
    return g;
  }, [stageEmps, flightRisks]);

  const bottlenecks = useMemo(() => {
    const issues = [];
    PIPELINE_STAGES.forEach(stage => PRODUCT_LINES.forEach(pl => {
      (grid[stage.id]?.[pl.id]?.warnings || []).forEach(w =>
        issues.push({ stage, pl, warning: w, emps: grid[stage.id][pl.id].emps })
      );
    }));
    const order = { critical: 0, spof: 1, flight: 2, understaffed: 3 };
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setFlowPos(prev => {
        ...
        if (n1 > PIPELINE_STAGES.length + 1) { setFlowPlaying(false); return { pl1: -1, pl2: -1 }; }
  ...
  const WARN_ICON = { critical: "🔴", spof: "🎯", flight: "✈️", understaffed: "⚠️" };

  const selStage = selectedCell ? PIPELINE_STAGES.find(s => s.id === selectedCell.stageId) : null;
  ...
  return (
    <div className="h-full flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="shrink-0 px-6 pt-4 pb-3 bg-white border-b border-gray-100">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Product Development Pipeline</h2>
            <p className="text-sm text-gray-500">Cross-functional team coverage across {PIPELINE_STAGES.length} semiconductor NPD stages · 2 active product lines</p>
          </div>
          <div className="flex items-center gap-2">
            {flowPlaying
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-3 text-xs text-gray-400 flex-wrap">
            {[["🔴","Critical gap"],["🎯","SPOF"],["⚠️","Understaffed"],["✈️","Flight risk"]].map(([icon,label]) => (
            ...
            {PIPELINE_STAGES.map((stage, sIdx) => {
              ...
              return (
                <div key={stage.id} style={{ minWidth: 144 }} className="flex flex-col">
                  {/* Stage header */}
                  <div className="rounded-t-xl px-2 py-2 text-center text-white" style={{ background: hdr }}>
            ...
            <>
              <div className="p-4 border-b border-gray-200 shrink-0" style={{ background: selPL.bg }}>
                ...
                <div className="flex gap-2 mt-3 text-xs flex-wrap">
                  ...
                  <span className="bg-white rounded-lg px-2 py-1 text-gray-500">stage {PIPELINE_STAGES.findIndex(s => s.id === selStage.id) + 1} of {PIPELINE_STAGES.length}</span>
                </div>
              </div>
                ...
                <div className="px-3 py-2 border-b border-gray-100 space-y-1 shrink-0">
                  {selCell.warnings.map((w, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs rounded-lg px-2 py-1.5"
                      style={{ background: w.type === "critical" ? "#fef2f2" : w.type === "spof" ? "#fff7ed" : w.type === "flight" ? "#f5f3ff" : "#fffbeb",
                               color:      w.type === "critical" ? "#dc2626"  : w.type === "spof" ? "#ea580c"  : w.type === "flight" ? "#7c3aed"  : "#d97706" }}>
                  ...
                  : bottlenecks.map((b, i) => (
                      <div key={i} onClick={() => setSelectedCell({ stageId: b.stage.id, plId: b.pl.id })}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-gray-400 mt-0.5" style={{ fontSize: 10 }}>Stage {PIPELINE_STAGES.findIndex(s => s.id === b.stage.id) + 1} · {b.stage.sublabel}</div>
                        </div>
      ...
      <div className="shrink-0 border-t border-gray-100 bg-white px-6 py-2 flex items-center gap-6 text-xs">
        ...
        <div className="ml-auto text-gray-400">Click any cell to inspect · "Simulate Flow" animates the product journey through stages</div>
      </div>
    </div>
  );
}
...
export default function OrgChartApp() {
  ...
  const handleDrop = useCallback((targetId) => {
    ...
    if (path.some(n => n.id === dragNode)) { setDragNode(null); setDragOver(null); teamDragRef.current = false; return; }
    ...
    setEmployees(prev => {
      const dragged  = prev.find(e => e.id === dragNode);
      const oldMgrId = dragged?.managerId ?? null;
      return prev.map(e => {
        if (e.id === dragNode) return { ...e, managerId: targetId };
        if (e.managerId === dragNode) {
          if (moveAsTeam) return e;                      // team: whole chain follows
          return { ...e, managerId: oldMgrId };          // individual: reports roll up
        }
        return e;
      });
    });
  ...
  const getTenureColor = useCallback((startDate) => {
    const months = (new Date() - new Date(startDate)) / (1000*60*60*24*30);
  ...
  const isFiltered = useCallback((node) => {
    ...
    return true;
  }, [filterDept, filterLoc, filterBG]);

  const departments = useMemo(() => [...new Set(activeEmployees.map(e => e.dept))].sort(), [activeEmployees]);
  const locations = useMemo(() => [...new Set(activeEmployees.map(e => e.location))].sort(), [activeEmployees]);
  ...
  const ctxValue = {
    ...
    activeEmployees, tree, hotspots, hotspotNodeIds, hotspotReasons, departments, locations, businessGroups,
    searchResults, displayRoot,
    toggleExpand, expandAll, collapseAll, navigateTo, handleDrop, undo,
    saveAnnotation, removeAnnotation, getNodeColor, getTenureColor, isFiltered, getTimelineMonths,
  };

  return (
    <AppCtx.Provider value={ctxValue}>
      <div className="h-screen flex overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif", background: "#f8fafc" }}>
        <style>{`
        ...
        {/* Sidebar */}
        <div className={`${sidebarCollapsed ? "w-14" : "w-56"} bg-slate-900 text-white flex flex-col shrink-0 transition-all duration-300`}>
                  ...
                  {MODES.map(m => {
                    ...
                    return (
                      <Tip key={m} text={meta.desc} side="right">
                        <button
                          onClick={() => { setMode(m); try { localStorage.setItem("orgSimMode", m); } catch {} if (!atLeast(m, MODE_LEVEL[view] > 0 ? "advanced" : "simple")) setView("org-chart"); }}
                ...
                {MODES.map(m => {
                  ...
                  return (
                    <Tip key={m} text={`${meta.label}: ${meta.desc}`} side="right">
                      <button
                        onClick={() => { setMode(m); try { localStorage.setItem("orgSimMode", m); } catch {}  }}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Top bar */}
          <div className="bg-white border-b border-gray-100 px-4 py-2 flex items-center gap-3 shrink-0 flex-wrap" style={{ boxShadow: "0 1px 2px rgba(0,0,0,0.03)" }}>
            {/* Search */}
            ...
            {view === "org-chart" && (
              <>
                ...
                <div className="flex items-center gap-1 border-l border-gray-200 pl-3">
                  ...
                  {["department", "business_group", "location"].map(c => (
                    <button key={c} onClick={() => setColorBy(c)} className={`text-xs px-2 py-1 rounded transition-colors ${colorBy === c ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}>
                      {c === "department" ? "Dept" : c === "business_group" ? "BU" : "Loc"}
                    </button>
                  ))}
                </div>
                ...
                <button onClick={() => setDragMode(m => m === "team" ? "individual" : "team")} className={`flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg transition-colors ${dragMode === "team" ? "bg-violet-600 text-white ring-2 ring-violet-300" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                  title="When ON, dragging any node moves its entire reporting chain">
            ...
            <div className="flex items-center gap-1 ml-auto border-l border-gray-100 pl-3">
              <button
                title="Export employees as CSV"
                onClick={() => {
                  const cols = ["id","first","last","title","dept","bg","fn","level","location","managerId","status","startDate","endDate","costCenter","band"];
                  const rows = [cols.join(","), ...employees.map(e => cols.map(c => `"${String(e[c]??"")}"`).join(","))];
          ...
          {/* Focus root breadcrumb */}
          {view === "org-chart" && (focusRoot && tree.map[focusRoot] || insightHighlightIds.size > 0) && (
            <div className="bg-blue-50 border-b border-blue-100 px-4 py-1.5 flex items-center gap-2 text-xs">
              <Eye size={12} className="text-blue-600"/>
              {focusRoot && tree.map[focusRoot] ? (
                <>
                  <span className="text-blue-700">Focused on subtree:</span>
                  {getAncestorPath(tree.map, focusRoot).map((n, i, arr) => (
                    <span key={n.id}>
                      <button onClick={() => setFocusRoot(i === 0 ? null : n.id)} className="text-blue-600 hover:underline">{n.first} {n.last}</button>
              ...
              <button onClick={() => { setFocusRoot(null); setInsightHighlightIds(new Set()); }} className="ml-auto text-blue-600 hover:text-blue-800 flex items-center gap-1 transition-colors"><X size={12}/>Show Full Org</button>
            </div>
          )}
              ...
              <div ref={treeContainerRef} className="h-full overflow-auto p-6 org-tree-scroll flex flex-col">
                ...
                <div className="flex items-center gap-2 mb-4 flex-wrap shrink-0">
                  ...
                  <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5 mr-2">
                    {[["tree","Tree"],["treemap","Treemap"],["sunburst","Sunburst"]].map(([m,l]) => (
